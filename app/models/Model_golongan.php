...
class Model_golongan extends CI_Model {
    ...
    /**
        ...
        * @description 	: Model_golongan model for administrator
    **/
 
    private $table       = 'tbl_golongan';
    private $id_table    = 'id_golongan';
    private $field_exist = 'kenaikan_ke';
    private $data_notif  = 'golongan';
    private $data_name   = 'kenaikan_ke';
    ...
    private function data_list()
    {
        ...
        if ($total > 0) {
            ...
            foreach ($query->result() as $row) {
                $akpbm  = $row->akk - $row->pd - $row->pi - $row->ki - $row->akp;
                $aksi   = '<span data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit"><a class="btn btn-sm btn-warning" href="' . base_url('' . $this->uri->segment(1) . '/edit/' . $this->myfunction->_encdec('enc', $row->id_golongan)) . '/" ><i class="fa fa-edit"></i></a></span>&nbsp;';
                $aksi  .= '<span data-toggle="tooltip" data-placement="top" title="" data-original-title="Hapus"><a href="#" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#confirm-delete" onClick="_get(\'' . $row->kenaikan_ke . '\')" data-href="' . base_url('' . $this->uri->segment(1) . '/proses/d/' . $this->myfunction->_encdec('enc', $row->id_golongan)) . '/" ><i class="fa fa-trash-o"></i></a></span>';
                $data[] = [
                    'no'          => $no, 
                    'kenaikan_ke' => $row->kenaikan_ke,
                    'akk'         => $row->akk,
                    'pd'          => $row->pd,
                    'pi'          => $row->pi,
                    'ki'          => $row->ki,
...
/* End of file Model_golongan.php */
/* Location: ./application/models/Model_golongan.php */