...
$(document).ready(function() {
    ...
    $(".jenis_laporan").change(function(){
        ...
        if(jenis_laporan == 'Pasien'){
            $(".field-pasien").show();
            $(".kategori_umur").attr('required', 'required');

            $(".field-penerima-layanan").hide();
            $(".id_spm").removeAttr('required', 'required');

            $(".field-tahun").hide();
            $(".tahun").removeAttr('required', 'required');
        }else if(jenis_laporan == 'Dokter'){
            $(".field-pasien").hide();
            $(".kategori_umur").removeAttr('required', 'required');
            ...
            $(".field-tahun").hide();
            $(".tahun").removeAttr('required', 'required');
        } else if (jenis_laporan == 'SPM') {
            $(".field-pasien").hide();
            $(".kategori_umur").removeAttr('required', 'required');

            $(".field-penerima-layanan").show();
            $(".id_spm").attr('required', 'required');
    ...
    $('.table-akta').DataTable({
        "ajax": {
            "url"       : _baseURL + "data-akta/get",
            "type"      : "POST",
            "data"      : {"type" : "all"},
        },
        "pageLength"    : 10,
        ...
        "columns": [
            ...
            { "data"    : "nama_pendaftar",  "sClass"  : "text-left"   }, 
            { "data"    : "no_akta",         "sClass"  : "text-center" },
            { "data"    : "tgl_nikah",       "sClass"  : "text-center" }, 
            { "data"    : "nama_penghulu",   "sClass"  : "text-left  " },
            { "data"    : "status",          "sClass"  : "text-center" }, 
            { "data"    : "aksi",            "sClass"  : "text-center" }
        ],
        "language": {
          "loadingRecords": "Loading.. <img src='"+_baseURL+"lib/assets/images/loader.gif'>",
          "emptyTable"    : "Data tidak ada.."
        }
    });
    $('.table-penghulu').DataTable({
        "ajax": {
            "url"       : _baseURL + "data-penghulu/get",
            "type"      : "POST",
            "data"      : {"type" : "all"},
        },
        "pageLength"    : 10,
        ...
        "columns": [
            { "data"    : "no",                "sClass"  : "text-center" },  
            { "data"    : "nama_penghulu",     "sClass"  : "text-center" },
            { "data"    : "jabatan_penghulu",  "sClass"  : "text-left"   }, 
    ...
    $('.table-registrasi').DataTable({
        "ajax": {
            "url"       : _baseURL + "data-registrasi/get",
            "type"      : "POST",
            "data"      : {"type" : "all", "status": localStorage.getItem("status")},
        },
        "pageLength"    : 10,
        "order"         : [[ 0, 'asc' ]],
        "columns": [
            ...
            { "data"    : "nama_lengkap",  "sClass"  : "text-left"   }, 
            { "data"    : "no_telp",       "sClass"  : "text-center" },
            { "data"    : "alamat",        "sClass"  : "text-left"   },
